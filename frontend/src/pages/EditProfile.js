...
const EditProfile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const response = await axios.get("http://127.0.0.1:8000/profile/edit/", {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put("http://127.0.0.1:8000/profile/edit/", dataToSend, {
                headers: {
        ...
        <div className="profile-edit-container">
            ...
            <form onSubmit={handleSubmit} className="profile-form">
                <div className="file-input-container">
                    ...
                    <div>
                        {data.avatarPreview ? (
                            <img src={data.avatarPreview} className="file-input-container-avatar" />
                        ) : data.avatar ? (
                            <img src={`http://127.0.0.1:8000${data.avatar}`} className="file-input-container-avatar" />
                        ) : (
                            <img 
                ...
                <div className="buttons">
                    ...
                    <button type="button" onClick={goToProfile}>Назад</button>
                </div>
            </form>
        </div>
    );
